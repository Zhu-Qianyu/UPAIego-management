...
### 群聊天室
- 成员在「群组」页聊天室交流；@豆小秘 可问制度、求跳转、（管理员）群发。
- 群发确认后公告气泡出现在聊天室，并同步个人收件箱。
`;
...
## 群聊天室模式
- 你出现在工作群**统一聊天室**时间线；用户通过 @豆小秘 唤起你。
- 回复适合**聊天气泡**：简洁分段，少用 Markdown 表格；单次回复建议不超过 600 字（除非用户要求全文规定）。
- 群发确认后公告会出现在聊天室；拟稿时说明「确认后将发到群聊并通知相关人员」。
- 用户仅问操作流程、未要求代填 → 用 assistant_message 解答 + actions 跳转，form_fills 为空。
...
async function fetchRecentGroupChatSummary(
  supabase: ReturnType<typeof createClient>,
  groupId: string,
  limit = 20
): Promise<string> {
  const { data, error } = await supabase
    ...
    .select("sender_type, content, message_kind, created_at")
    .eq("group_id", groupId)
    .order("created_at", { ascending: false })
    .limit(limit);

  if (error || !data?.length) {
    return "【群聊最近消息】暂无或表未创建。";
  }
    ...
    const who =
      row.sender_type === "bot"
        ...
        : row.sender_type === "system"
          ? "系统"
          : "成员";
    const kind = row.message_kind === "broadcast" ? "[公告]" : "";
    const text = String(row.content ?? "").replace(/\s+/g, " ").trim().slice(0, 120);
...
Deno.serve(async (req) => {
  ...
  const groupChatSummary = await fetchRecentGroupChatSummary(supabase, body.groupId);
