...
CREATE POLICY "wg_select_related"
  ...
  USING (
    ...
    OR id = public.user_active_group_id()
    OR id = public.user_owned_group_id()
    OR EXISTS (
      SELECT 1 FROM public.group_members gm WHERE gm.group_id = work_groups.id AND gm.user_id = auth.uid()
    )
...
CREATE POLICY "wg_insert_one_owned"
  ...
  WITH CHECK (
    ...
    AND NOT EXISTS (SELECT 1 FROM public.work_groups w2 WHERE w2.owner_user_id = auth.uid())
  );
...
CREATE POLICY "gm_select"
  ...
  USING (
    user_id = auth.uid()
    OR EXISTS (SELECT 1 FROM public.work_groups g WHERE g.id = group_members.group_id AND g.owner_user_id = auth.uid())
    OR public.current_profile_role() = 'admin'
    OR EXISTS (
      SELECT 1 FROM public.group_members o
      WHERE o.group_id = group_members.group_id AND o.user_id = auth.uid() AND o.membership_status = 'active'
    )
  );

CREATE POLICY "gm_update_group_owner_review"
  ON public.group_members FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.work_groups g WHERE g.id = group_members.group_id AND g.owner_user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.work_groups g WHERE g.id = group_members.group_id AND g.owner_user_id = auth.uid()));
